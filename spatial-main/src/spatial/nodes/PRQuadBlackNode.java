...
public class PRQuadBlackNode extends PRQuadNode {
    ...
    public PRQuadNode insert(KDPoint p, int k) {
    	//System.out.printf("Insert with x = %d, y = %d\n", p.coords[0], p.coords[1]);
    	
        if(this.bucket.size() < this.bucketingParam) {
        	this.bucket.add(p);
        	return this;
        }else {
        	PRQuadNode splitGrayNode = new PRQuadGrayNode(this.centroid, k, this.bucketingParam);
        	for(int i = 0; i < this.bucket.size(); i++) {
        		splitGrayNode.insert(this.bucket.get(i), k);
        	}
        	splitGrayNode.insert(p, k);
        	return splitGrayNode;
        }
    }
    ...
    public void range(KDPoint anchor, Collection<KDPoint> results,
                      double range) {
        for(int i = 0; i < this.bucket.size(); i++) {
        	if(this.bucket.get(i).euclideanDistance(anchor) <= range && this.bucket.get(i).equals(anchor) == false) {
    ...
    public NNData<KDPoint> nearestNeighbor(KDPoint anchor, NNData<KDPoint> n) {
        for(int i = 0; i < this.bucket.size(); i++) {
        	if(n.getBestDist() == -1 || this.bucket.get(i).euclideanDistance(anchor) < n.getBestDist()) {
        		if(this.bucket.get(i).equals(anchor) == false) {
        			n.update(this.bucket.get(i), this.bucket.get(i).euclideanDistance(anchor));
        		}
        	}
        }
        return n;
    ...
    public void kNearestNeighbors(int k, KDPoint anchor, BoundedPriorityQueue<KDPoint> queue){
        for(int i = 0; i < this.bucket.size(); i++) {
        	if(this.bucket.get(i).equals(anchor) == false) {
        		queue.enqueue(this.bucket.get(i), this.bucket.get(i).euclideanDistance(anchor));
        	}
        }
    }
}
