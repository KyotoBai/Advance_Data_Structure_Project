...
public class OrderedLinearProbingHashTable extends OpenAddressingHashTable {
    ...
    public Probes put(String key, String value) {
        ...
        //find the position where could insert
        int k_val = this.hash(key);
        KVPair insert_kv = new KVPair(key, value), curr = null;
        
        while(this.table[k_val] != null) {
        	if(this.table[k_val].getKey().compareTo(insert_kv.getKey()) > 0 && this.table[k_val] != TOMBSTONE) {
        		curr = this.table[k_val];
        		this.table[k_val] = insert_kv;
        		insert_kv = curr;
        	}
        	probe_count++;
    ...
    public Probes get(String key) {
    	...
    	while(this.table[k_val] != null) {
    		// if key should be insert here
    		if(this.table[k_val].getKey().compareTo(key) >= 0 && this.table[k_val] != TOMBSTONE) {
    			// if the there isn't same key, break
    ...
    public Probes remove(String key) {
    	...
    	if(this.softFlag == true) {
    		...
    		while(this.table[k_val] != null) {
    			// key should insert in this domain
    			if(this.table[k_val].getKey().compareTo(key) >= 0 && this.table[k_val] != TOMBSTONE) {
    				// if key was found in this domain
    				if(!this.table[k_val].getKey().equals(key)) {
    					break;
    				}
